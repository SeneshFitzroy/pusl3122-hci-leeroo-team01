      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col gap-6">
            {/* Filter row */}
            <div>
              <p className="text-xs font-semibold text-darkwood dark:text-warm-100 uppercase tracking-wider mb-3">Filter by status</p>
                  ...
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all inline-flex items-center gap-1.5 ${
                      filterStatus === id
                        ...
                        : 'bg-white dark:bg-dark-card text-darkwood/70 dark:text-warm-200 border border-warm-200 dark:border-dark-border hover:border-clay hover:text-clay dark:hover:text-clay'
                    }`}
                  >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="relative flex-1 sm:max-w-xs">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-darkwood dark:text-warm-200" />
                <input
                  type="text"
                  ...
                  className="w-full pl-10 pr-4 py-2.5 border border-warm-200 dark:border-dark-border bg-white dark:bg-dark-card rounded-xl text-sm text-darkwood dark:text-warm-100 placeholder:text-warm-400 dark:placeholder:text-warm-200 focus:ring-2 focus:ring-clay/30 focus:border-clay outline-none"
                />
              </div>
              <select
                value={sortBy}
              ...
              <div className="flex items-center gap-2">
                ...
                {filteredDesigns.length > 0 && (
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl border border-warm-200 dark:border-dark-border text-darkwood/70 dark:text-warm-200 hover:border-clay hover:text-clay hover:bg-clay/5 dark:hover:bg-clay/10 transition-all inline-flex items-center gap-2"
            ...
            <div className="space-y-3">
              {filteredDesigns.length === 0 ? (
                <div className="bg-white dark:bg-dark-card rounded-2xl p-10 border border-warm-200 dark:border-dark-border text-center">
                  <FolderOpen className="h-14 w-14 text-warm-300 dark:text-warm-200 mx-auto mb-4 opacity-60" />
                  <p className="text-darkwood dark:text-warm-200 mb-1 font-medium">
                    {designs.length === 0 ? 'No designs yet' : 'No designs match filters'}
                  </p>
                  <p className="text-sm text-darkwood/70 dark:text-warm-200 mb-6">
                ...
                filteredDesigns.map(design => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-darkwood/70 dark:text-warm-200 mb-3 flex items-center gap-1.5">
                        <Users className="h-3 w-3" />
                        {design.userName || 'Customer'}
                      </p>
                      <div className="flex items-center gap-4 text-xs text-darkwood/60 dark:text-warm-200">
          ...
          <div className="lg:col-span-2">
            {selectedDesign ? (
              <div className="bg-white dark:bg-dark-card rounded-2xl border border-warm-200 dark:border-dark-border overflow-hidden shadow-sm">
                ...
                <div className="p-6 sm:p-8 border-b border-warm-200 dark:border-dark-border bg-warm-50/50 dark:bg-dark-surface/80">
                  ...
                  <div className="mb-4 p-4 bg-clay/10 dark:bg-clay/5 rounded-xl border border-clay/20">
                    ...
                    <p className="text-xs text-darkwood/60 dark:text-warm-200 mb-3">Send a link instead of exporting PDF. Client opens the design directly in the editor.</p>
                    <button
                  ...
                  <div className="mt-6 pt-6 border-t border-warm-200 dark:border-dark-border">
                    <p className="text-xs font-semibold text-darkwood dark:text-warm-100 uppercase tracking-wider mb-3">Update status</p>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="p-6 sm:p-8 border-b border-warm-200 dark:border-dark-border">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="bg-warm-50 dark:bg-dark-surface rounded-xl p-4 text-center">
                      ...
                      <div className="text-xs text-darkwood dark:text-warm-200 font-medium mt-0.5">Rooms</div>
                    </div>
                    <div className="bg-warm-50 dark:bg-dark-surface rounded-xl p-4 text-center">
                      ...
                      <div className="text-xs text-darkwood dark:text-warm-200 font-medium mt-0.5">Items</div>
                    </div>
                    <div className="bg-warm-50 dark:bg-dark-surface rounded-xl p-4 text-center">
                      <div className="text-xl font-bold text-darkwood dark:text-warm-100">
                        {selectedDesign.roomWidth || 5}m × {selectedDesign.roomDepth || 4}m
                      </div>
                      <div className="text-xs text-darkwood dark:text-warm-200 font-medium mt-0.5">Dimensions</div>
                    </div>
                    <div className="bg-warm-50 dark:bg-dark-surface rounded-xl p-4 text-center">
                      <div className="text-xl font-bold text-darkwood dark:text-warm-100">
                        {new Date(selectedDesign.createdAt || Date.now()).toLocaleDateString()}
                      </div>
                      <div className="text-xs text-darkwood dark:text-warm-200 font-medium mt-0.5">Created</div>
                    </div>
                  </div>
                        ...
                        {selectedDesign.rooms.map((room, idx) => (
                          <div key={idx} className="bg-warm-50 dark:bg-dark-surface rounded-xl p-4">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-xs text-darkwood dark:text-warm-200">({room.furnitureItems?.length || 0} items)</span>
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {room.furnitureItems?.map((item, fi) => (
                                <span key={fi} className="px-2.5 py-1 bg-white dark:bg-dark-card rounded-lg text-xs text-darkwood/70 dark:text-warm-200 border border-warm-200 dark:border-dark-border">
                                  {item.name}
                                </span>
                              ))}
                            </div>
                ...
                <div className="p-6 sm:p-8 border-b border-warm-200 dark:border-dark-border bg-amber-50/30 dark:bg-amber-900/5">
                  ...
                  <p className="text-xs text-darkwood dark:text-warm-200 mb-3">Client preferences, budget notes, follow-up reminders — not visible to clients.</p>
                  <textarea
                    value={designerNotes}
                    onChange={(e) => setDesignerNotes(e.target.value)}
                    placeholder="e.g. Client prefers natural wood finish. Follow up next week."
                    rows={3}
                    className="w-full px-4 py-3 border border-warm-200 dark:border-dark-border bg-white dark:bg-dark-card rounded-xl text-sm text-darkwood dark:text-warm-100 placeholder:text-warm-400 dark:placeholder:text-warm-200 focus:ring-2 focus:ring-clay/30 outline-none resize-none"
                  />
                  <button
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <div className="space-y-4 mb-6 max-h-72 overflow-y-auto">
                    {(!selectedDesign.comments || selectedDesign.comments.length === 0) && (
                      <div className="text-center py-10 rounded-xl bg-warm-50 dark:bg-dark-surface/50 border border-dashed border-warm-200 dark:border-dark-border">
                        <MessageSquare className="h-10 w-10 mx-auto mb-3 text-warm-300 dark:text-warm-200 opacity-60" />
                        <p className="text-sm font-medium text-darkwood dark:text-warm-100 mb-1">No comments yet</p>
                        <p className="text-xs text-darkwood dark:text-warm-200">Add your first review below.</p>
                      </div>
                    )}
                    {selectedDesign.comments?.map((c, ci) => (
                      <div key={ci} className="bg-warm-50 dark:bg-dark-surface rounded-xl p-4 border border-warm-100 dark:border-dark-border">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs text-darkwood dark:text-warm-200">
                            {c.date ? new Date(c.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}
                          </span>
                        </div>
                        <p className="text-sm text-darkwood/70 dark:text-warm-200 leading-relaxed">{c.text}</p>
                        {c.rating && (
                  ...
                  <div className="border-t border-warm-200 dark:border-dark-border pt-6">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-3 border border-warm-200 dark:border-dark-border bg-warm-50 dark:bg-dark-surface rounded-xl text-sm text-darkwood dark:text-warm-100 placeholder:text-warm-400 dark:placeholder:text-warm-200 focus:ring-2 focus:ring-clay/30 focus:border-clay outline-none resize-none mb-4"
                    />
                    <button
              ...
              <div className="bg-white dark:bg-dark-card rounded-2xl border border-warm-200 dark:border-dark-border p-16 text-center">
                <Eye className="h-20 w-20 text-warm-400 dark:text-warm-200 mx-auto mb-5 opacity-70" />
                <h3 className="text-xl font-bold text-darkwood dark:text-warm-100 mb-2">Select a design to review</h3>
                <p className="text-darkwood/70 dark:text-warm-200 max-w-sm mx-auto leading-relaxed">
